...
function npcGen(buildingObj, first) {
  ...
  for (let i = 0; i < buildingList.length; i++) {
    if (building === buildingList[i].building) {
      buildingCheck = 1;
    }
  }
  if (buildingCheck === 0) {
    return console.log('Building not in list given');
  }
  // id generation
  let firstNPC = first;
  ...
  if (randInt < humanChance) { // is number between 0 and (humanChance)
    ...
    raceNPC = raceName[1];
  } else if (randInt < (humanChance + elfChance + dwarfChance)) { // is number between (elfChance) & (dwarfChance)
    raceNPC = raceName[2];
...
npcGen('Orphanage');// test child generation
npcGen('Home');// test invalid building (building not in list)
